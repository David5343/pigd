<div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-lg">
    ...
    <form wire:submit ="guardar" class="flex flex-wrap gap-2">
        <div class="flex flex-col w-full md:w-full">
            <h5 class="text-base font-bold text-gray-700 mb-4">Ingresa los datos del usuario,selecciona el Rol y Los
                Permisos
            </h5>
        </div>
        ...
        <!-- Dropdown de Roles -->
        <div class="flex flex-col w-full md:w-1/3">
            <select wire:model.live="selectedRole" id="role" name="role"
                class="border rounded-lg px-3 py-2 w-full focus:ring focus:ring-blue-200">
        ...
        <!-- Lista de Permisos Dinámica -->
        @if (!empty($permissions))
            <div class="flex flex-col w-full">
                <h2 class="text-xl font-bold text-gray-700 mb-4 mt-4">Permisos</h2>
            </div>

            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach ($permissions as $permission)
                    <div class="flex items-center gap-2">
                        <input wire:model.live="selectedPermissions" type="checkbox" value="{{ $permission->id }}"
                            id="permission_{{ $permission->id }}" wire:key="perm_{{ $permission->id }}"
