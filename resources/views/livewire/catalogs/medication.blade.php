<div>
    <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
        ...
        <div class="text-right">
            <a href="{{ route('medication.import') }}"
                        ...
                        <table class="min-w-full text-left text-sm font-light text-surface dark:text-white">
                            <thead class="border-b border-neutral-200 font-medium dark:border-white/10">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4">Número de lote</th>
                                    <th scope="col" class="px-6 py-4">Nombre comercial</th>
                                    <th scope="col" class="px-6 py-4">Unidad de medida</th>
                                    <th scope="col" class="px-6 py-4">Proveedor</th>
                                    <th scope="col" class="px-6 py-4">Precio</th>
                                    <th scope="col" class="px-6 py-4">Año del Catalogo</th>
                                    <th scope="col" class="px-6 py-4">Agregar precio</th>
                                ...
                                @if ($lista->count())
                                    @foreach ($lista as $item)
                                        <tr class="border-b border-neutral-200 dark:border-white/10">
                                            ...
                                            <td class="whitespace-nowrap px-6 py-4">
                                                {{ $item->medicationUnit->name ?? 'N/A'}}
                                            </td>
                                            <td class="whitespace-nowrap px-6 py-4 font-medium">{{ $item->supplier->name }}</td>
                                            <td class="whitespace-nowrap px-6 py-4 font-medium">{{ $item->currentPrice()?->unit_price;}}</td>
                                            <td class="whitespace-nowrap px-6 py-4 font-medium">{{ $item->currentPrice()?->catalogYear->year;}}</td>
                                            <td class="px-2 py-1 text-center">
                                                <button
                                                    class="px-2 py-1 rounded text-sm
                                                        {{ auth()->user()->can('medication.set.price')
                                                            ? 'bg-blue-500 text-white hover:bg-blue-600'
                                                            : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                                                        }}"
                                                    @if (!auth()->user()->can('medication.set.price')) disabled @endif
                                                    wire:click="$dispatchTo('catalogs.medication-set-price','openModalPrice', { medicationId: {{ $item->id }} })">
                                                    Agregar precio
                                                </button>
                                            </td>
                                            <td class="whitespace-nowrap px-6 py-4">
                                                <a
                                                    href="{{ url('medical_coordination/catalogs/medication_unit/' . $item->id . '/edit') }}">
        ...
        <div class="flex items-center space-x-2 w-full md:w-auto">
            ...
            <livewire:catalogs.medication-set-price />
    </div>
