<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Recursos Humanos/Catalogos/Grado de estudio/Editar') }}
        </h2>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <form method="POST" class="flex flex-wrap gap-2"
                        action="{{ url('human_resources/catalogs/degrees/' . $row->id) }}">
                        @method('PUT')
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="flex flex-col w-full md:w-1/12">
                            ...
                            <input type="text" value="{{ old('abbreviation', $row->abbreviation) }}"
                                id="abbreviation" name="abbreviation" required
                                class="border rounded-lg px-3 py-2 w-full focus:ring focus:ring-blue-200">
                            @error('abbreviation')
                                <div class="bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-400 p-2 rounded-lg mt-2"
                                    role="alert">
