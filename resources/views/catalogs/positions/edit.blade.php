<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Recursos Humanos/Catalogos/Puestos/Editar') }}
        </h2>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <form method="POST" class="flex flex-wrap gap-2"
                        action="{{ url('human_resources/catalogs/positions/' . $row->id) }}">
                        @method('PUT')
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="flex flex-col w-full">
                            <div class="flex flex-col w-full md:w-1/6">
                                ...
                                <input type="text" id="position_number" name="position_number"
                                    value="{{ $row->position_number }}" maxlength="3" required
                        ...
                        <div class="flex flex-col w-full">
                            <div class="flex flex-col w-full md:w-1/2">
                                <label for="salary" class="text-sm font-medium text-gray-700">* Nombre del
                                    puesto</label>
                                <input type="text" id="position_name" name="position_name" maxlength="80"
                                    value="{{ $row->position_name }}" required
                                    class="border rounded-lg px-3 py-2 w-full focus:ring focus:ring-blue-200">
                            </div>
                            @error('position_name')
                                <div class="w-full bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-400 p-4 rounded-lg mt-2"
                                ...
                                <select id="category_id" name="category_id"
                                    ...
                                    @foreach ($categories as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $item->id == $row->category_id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
