<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Recursos Humanos/Catalogos/Bancos/Editar') }}
        </h2>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <form method="POST" class="flex flex-wrap gap-2"
                        action="{{ url('human_resources/catalogs/banks/' . $row->id) }}">
                        @method('PUT')
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="flex flex-col w-full">
                            <div class="flex flex-col w-full md:w-1/5">
                                ...
                                <input type="text" id="key" name="key"
                                    ...
                                    value="{{ old('key', $row->key) }}" required>
                            </div>
                        </div>
                        <div class="flex flex-col w-full">
                        ...
                        <div class="flex flex-col w-full">
                            <div class="flex flex-col w-full md:w-full">
                                <label for="nombre" class="text-sm font-medium text-gray-700">* Razón Social</label>
                                <textarea id="legal_name" name="legal_name" rows="3" minlength="3" maxlength="120"
                                    class="border rounded-lg px-3 py-2 w-full focus:ring focus:ring-blue-200">{{ old('legal_name', $row->legal_name) }}</textarea>
                            </div>
                        </div>
