<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Recursos Humanos/Catalogos/Tipo de contrato laboral/Editar') }}
        </h2>
    </x-slot>
    <div class="py-12">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-lg">
                    ...
                    <form method="POST" class="flex flex-wrap gap-2"
                        action="{{ url('human_resources/catalogs/contract-type/' . $row->id) }}">
                        @method('PUT')
                        @csrf
                        @if ($errors->any())
                        ...
                        <div class="flex flex-col w-full md:w-1/3">
                            ...
                            <input type="text" value="{{ old('name', $row->name) }}" id="name" name="name"
                                placeholder="Nuevo Ingreso" required
                        ...
                        <div class="flex flex-col w-full">
                            ...
                            <textarea type="text" id="description" name="description"
                                class="border rounded-lg px-3 py-2 w-full focus:ring focus:ring-blue-200">{{ old('description', $row->description) }}</textarea>
                            @error('description')
                                <div class="bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-400 p-2 rounded-lg mt-2"
                                    role="alert">
                                    {{ $message }}
